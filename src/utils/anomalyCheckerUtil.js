...
const anomalyCheckerUtil = (
  ...
  relationsRegistrationsToEPR.forEach((deviation, index) => {
    if (
      ratioRegistrationsToEPR - permissibleDeviation <= deviation &&
      deviation <= ratioRegistrationsToEPR + permissibleDeviation
    ) {
      stepDifferenceUtil(medianStepDeviation, index, CSVData)
